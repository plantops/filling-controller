...
//
// The weight model is deliberately crude. It exists to let the FSM traverse
// CoarseFill -> FineFill -> Cutoff -> Settle at a watchable pace, not to
// represent the plant. Nothing measured here says anything about real filling
// accuracy.
...
// Simulated fill rates, kg per second.
constexpr float kCoarseRate = 8.0F;
constexpr float kFineRate = 1.2F;

// Bag is considered emptied and the scale re-zeroed at these states.
...
void controller_task(void*) {
    ...
    for (;;) {
        ...
        // DI3 machine.motor_running: ON = AUTO, OFF = MANUAL.
        inputs.mode = input(inputs, Di::MachineMotorRunning)
                          ? OperationMode::Auto
                          : OperationMode::Manual;

        // Simulated plant: weight rises only while the FSM is filling.
        ...
        const ControllerSnapshot snap = controller.tick(now_us, inputs, weight);

        if (snap.state != state) {
            ESP_LOGI(kTag, "%s -> %s  fault=%s weight=%.3f kg cycle=%lu",
                     state_name(state), state_name(snap.state),
                     fault_name(snap.fault), static_cast<double>(net_kg),
